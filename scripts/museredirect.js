...
Muse.Redirect.redirect = function (a, b, c) {
    if (Muse.Redirect.readCookie("inbrowserediting") != "true") {
        var d = null,
            f = Muse.Redirect.readCookie("devicelock");
        f == "phone" && c ? d = c : f == "tablet" && b && (d = b);
        if (f != a && d == null) {
            var a = Math.min(screen.width, screen.height) / (window.devicePixelRatio || 1),
                ...
                f = f > 0 && g > 0 ? Math.min(screen.width / f, screen.height / g) : 0;
            (a <= 370 || f != 0 && f <= 3) && c ? d = c : a <= 960 && b && Muse.Redirect.Touch && (d = b)
        }
        if (d != null) document.location = d, document.write('<style type="text/css">body {visibility:hidden}</style>')
    }
};;
(function () {
        ...
        if (-1 != a) {
            ...
            for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
                ...
                if ("generator" == d.getAttribute("name")) {
                    "2015.0.2.310" != d.getAttribute("content") && Muse.assets.outOfDate.push("museredirect.js");
                    break
                }
            }
        }
