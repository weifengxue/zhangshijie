...
Muse.Utils.anyStyleSheetRule = function (a, b) {
    ...
    for (var f = 0; f < d.length; ++f) {
        var l = d[f];
        if (Muse.Utils.isDefined(l.selectorText))
            if (c) {
                if (b(l.selectorText)) return l
            } else
                for (var h = l.selectorText.split(/\s*,\s*/), k = 0; k < h.length; k++)
                    if (b(h[k])) return l
    }
    return null
};
    ...
    if (!d || d == "_self") {
        ...
        if (f && d > g) {
            var a = c.data("scrollWrapper"),
                l = b.substring(d),
                d = Muse.Utils.getAnchorWithDestination(l).offset(),
                b = f[1],
                h = a || window,
                ...
                k = Math.min(f, d.top + (a && !a.isStandard() ? a.scrollTop() :
                    0)),
...
Muse.Utils.enableAnchorLinksActiveState = function () {
    ...
    if (0 != c.length) {
        var l = !1,
            ...
            i = function () {
                l = !1;
                var a = b ? k.scrollLeft() : k.scrollTop(),
                    d;
                a: {
                    var i = 0;d = c.length;
                    for (var j; i < d; i++)
                        if (j = c[i], j.from <= a && a <= j.to) {
                            d = i;
                            break a
                        } d = -1
                }
                ...
                for (d = Math.min(d + 2, c.length); i < d; i++)
                    if (m = c[i], j = m.$elem.offset().left + (h && !h.isStandard() ? h.scrollLeft() : 0), q = m.$elem.offset().top + (h && !h.isStandard() ? h.scrollTop() : 0), m.from != (b ? j : q)) {
                        f();
                        break
                    } i = 0;
                for (d =
...
Muse.Utils.saveHyperlinkInfo = function (a, b, c, d, g) {
    var f = a.attr("href"),
        l = Muse.Utils.pathOnly(f),
        h = -1,
        k = a.attr("target"),
        i = window.location.href.replace(/#.*$/i, "");
    if (f && -1 != f.indexOf("#") && !(k && k != "_self") && !(0 <= l.indexOf("/")) && (i.charAt(i.length - 1) == "/" && (i += "index.html"), -1 != i.indexOf("/" + l, i.length - l.length - 1))) {
        var l = $(window).data("scrollWrapper"),
            j = f.substring(f.lastIndexOf("#")),
