...
(function (a) {
    ...
    d.prototype.initialize = function () {
        ...
        for (var a, b = 0, c; c = this.data[b]; b++) c.length = c["in"][1] - c["in"][0], c.startPosition = a ? {
    ...
    d.prototype.update = function (a, b, c) {
        var g = this.elementLeft - c.scrollLeft,
            f = this.elementTop - this.referenceOffset,
            h = a.startPosition.left + a.speed[0] * b,
            b = a.startPosition.top +
            a.speed[1] * b,
            j = {};
        if ("number" == typeof a.speed[0]) j.left = g + h + "px";
        if ("number" == typeof a.speed[1]) j.top = f - b + "px";
        if (this.visible = this.skipVisibleCheck || this.getVisible(g + h, f - b, c)) {
            if (this.isMarkedAsOOV) j.display = "", this.isMarkedAsOOV = !1;
            this.cssProxy.setCSSProperties(this.elem, j)
        } else if (!this.isMarkedAsOOV) this.cssProxy.setCSSProperties(this.elem, {
            display: "none"
        }), this.isMarkedAsOOV = !0;
        if (this.hasPreInitClass) this.elem.removeClass(d.HIDDEN_CLASS), this.hasPreInitClass = !1
    };
    d.prototype.getVisible = function (a,
        b, c) {
        var d = Math.max(this.elemWidth, this.elemHeight) + 100;
        return (void 0 === a || a + d > 0 && a - d < c.windowWidth) && (void 0 === b || b + d > 0 && b - d < c.windowHeight)
    };
    var f = function (a, b, c) {
        ...
        this.cssProxy = this.service.cssProxy;
        this.r7Mode = !0;
        if (!this.r7Mode && (this.cssBackgroundPosition = this.elem.css("background-position"), this.cssBackgroundPosition.match(/^\d+\%$/gi))) this.cssBackgroundPosition = (a = this.elem[0].currentStyle) && a.backgroundPositionX && a.backgroundPositionY ? a.backgroundPositionX +
    ...
    f.BG_COVER = 1;
    f.BG_CONTAIN = 2;
    f.prototype.getCSSRule = function () {
        if (!this.pageSheet) this.pageStyleSheet = Muse.Utils.getPageStyleSheet();
        if (!this.cssRule) this.cssRule = Muse.Utils.getStyleSheetRuleById(this.pageStyleSheet,
            this.elem.attr("id"));
        return this.cssRule
    };
    ...
    f.prototype.initialize = function () {
        ...
        this.hasPositionEffect = (this.positionEffect = this.service.getElementEffect(this.is100PercentWidth ?
            b : this.elem, d)) && this.positionEffect.enabled;
        for (var b = 0, g, f; g = this.data[b]; b++) g.speed[0] -= 0, g.speed[1] -= 1, g.length = g["in"][1] - g["in"][0], g.startPosition = null == f ? {
            left: -g.length * g.speed[0],
            top: -g.length * g.speed[1]
        } : {
            left: f.startPosition.left + f.length * f.speed[0],
            top: f.startPosition.top + f.length * f.speed[1]
        }, f = g;
        if (!Muse.Browser.Features.checkCSSFeature("background-size") && this.elem.hasClass("museBGSize") && 0 < a("> .museBgSizePolyfill", this.elem).length) this.polyfillElement = a(a(".museBgSizePolyfill img",
            this.elem)[0]);
        ...
        if (this.elem.hasClass("browser_width")) this.originalWidth = Muse.Utils.tryParse(Muse.Utils.getRuleProperty(this.getCSSRule(), "width"), parseInt)
    };
    f.prototype.onWindowResize = function (a) {
        a.data.recalculateBackgroundOffset = !0
    };
    f.prototype.onElementResize =
        function (a) {
            var a = a.data,
    ...
    f.prototype.updateFixedBackground = function (a, b) {
        var c = this.getBackgroundModeDisplayRatio(),
            d = this.elem.offset(),
            g = d.left,
            h = d.top - this.referenceOffset;
        if (this.hasPositionEffect && 0 == this.positionEffect.data[this.data.indexOf(a)].speed[1] || !this.hasPositionEffect && "fixed" ==
            this.elem.css("position")) h = d.top - (a["in"][0] + b);
        d = (f.BG_COVER !== this.bgMode || !this.is100PercentWidth ? g : 0) + this.backgroundPosition.multiplier.x * (this.elem.width() - c * this.backgroundSize.width) + Muse.Utils.getCSSIntValue(this.elem, "border-left-width");
        ...
        h = {
            "background-position": d + "px " + h + "px"
        };
        1 != c && (h["background-size"] = c * this.backgroundSize.width + "px " + c *
            this.backgroundSize.height + "px");
        this.cssProxy.setCSSProperties(this.elem, h)
    };
    f.prototype.update = function (a, b) {
        if (this.backgroundOffsetAvailable) {
            if (this.recalculateBackgroundOffset) this.recalculateBackgroundOffset = !1, this.getBackgroundOffset();
            if (this.backgroundFixedMode) this.updateFixedBackground(a, b);
            else {
                var c = this.getBackgroundModeDisplayRatio() - 1,
                    d = Math.floor(this.bgOffset.x - c * this.backgroundPosition.multiplier.x * this.backgroundSize.width + this.getDeltaWidth()) + a.startPosition.left + a.speed[0] *
                    b,
        ...
        } else this.updateRequested = !0
    };
    f.prototype.getBackgroundOffset = function () {
        var a = Muse.Utils.tryParse(this.backgroundPosition.x,
                parseFloat, 0),
        ...
        if (!Muse.Utils.endsWith(this.backgroundPosition.x, "%") && !Muse.Utils.endsWith(this.backgroundPosition.y, "%")) this.onBackgroundOffsetAvailable(a, b);
        else if (this.backgroundSize) this.updateBackgroundOffset(a, b);
        else {
            var c = this;
            this.getBackgroundSize(function (d) {
                ...
                if (c.updateRequested) {
                    c.updateRequested = !1;
                    var d = c.service.getEffectProgress(),
                        g = c.service.getEffectInterval(c, d);
                    c.update(g,
                        d - g["in"][0])
                }
            })
    ...
    f.prototype.updateBackgroundOffset = function (a, b) {
        var c = this.is100PercentWidth && this.hasPositionEffect && this.positionEffect.isMarkedAsOOV ? this.elem.parent() : this.elem;
        if (Muse.Utils.endsWith(this.backgroundPosition.x, "%")) var d = Muse.Utils.firstDefined(this.originalWidth, c.innerWidth()),
            a = a / 100 * (d - Muse.Utils.firstDefined(this.backgroundSize.width, d));
        Muse.Utils.endsWith(this.backgroundPosition.y, "%") && (c = c.innerHeight(), b = b / 100 * (c - Muse.Utils.firstDefined(this.backgroundSize.height,
            c)));
    ...
    f.prototype.isValidBackgroundPosition =
        function (a) {
    ...
    f.prototype.getBackgroundPosition = function () {
        ...
        a = a.replace(/(?:left|top)/gi, "0%").replace(/center/gi,
            "50%").replace(/(?:right|bottom)/gi, "100%");
        a = a.replace(/^\s+|\s+$/gi, "");
        a = a.split(" ");
    ...
    var j = function (a, b, c) {
        ...
        this.data = c;
        this.cssProxy = this.service.cssProxy;
        this.elemToBeMarkedAsInvisible = this.elem.parent().hasClass("browser_width") ?
            this.elem.parent() : this.elem;
    ...
    j.prototype.initialize = function () {
        ...
        0 < c.fade &&
            (c["in"][0] += c.fade, this.data.splice(this.data.length - 1, 0, {
    ...
    j.prototype.setElementOpacity = function (a) {
        ...
        var b = 0 === a;
        if (void 0 === this.previousOpacity || b && 0 !== this.previousOpacity || !b && 0 === this.previousOpacity) b ?
            this.elemToBeMarkedAsInvisible.addClass(j.INVISIBLE_CLASS_NAME) : this.elemToBeMarkedAsInvisible.removeClass(j.INVISIBLE_CLASS_NAME);
        this.previousOpacity = a
    };
    j.prototype.update = function (a, b) {
        var c = 0;
        if (!this.hasPositionEffect || this.positionEffect.visible) c = "number" != typeof a.opacity ? a.opacity[0] + a.rate * b : a.opacity, c = Math.max(Math.min(c, 100), 0);
        this.setElementOpacity(c);
        if (this.hasPreInitClass) this.elem.removeClass(j.PRE_INITIT_CLASS_NAME), this.hasPreInitClass = !1
    };
    ...
    h.prototype.initialize = function () {
        ...
        if (this.isLinkToScrollEffect = this.isLinkToScrollInterval(this.data[1])) this.data[1].intervalLength = this.data[1]["in"][1] - this.data[1]["in"][0], Muse.Assert.assert(2 == this.data.length || Infinity != this.data[1].intervalLength, "In a 3 interval configuration, why do we have middle interval with length = Infinity?")
    };
    h.prototype.update = function (a, b) {
        if (this.play !== a.play) !0 === a.play ? (this.play = !0, this.start()) : !1 === a.play ? (this.play = !1, this.stop()) : this.isLinkToScrollInterval(a) ? (this.play = void 0, this.jump(b)) : Muse.Assert.assert(!1, "Unknown widget configuration: play=" + a.play);
    ...
    h.prototype.start = function () {
        ...
        a(this.widget).one("wp-slideshow-play",
            function () {
    ...
    l.prototype.initialize = function () {
        ...
        Muse.Assert.assert(2 == this.data.length || Infinity != this.data[1].intervalLength,
            "In a 3 interval configuration, why do we have middle interval with length = Infinity?");
    ...
    l.prototype.updateStage = function (a) {
    ...
    l.prototype.onCompositionReady =
        function (a, b) {
            var c = b.iframeWindow.AdobeEdge,
                d = null;
            Muse.Assert.assert(null != c, "AdobeEdge object must not be null.");
            "undefined" != typeof c.compositions ? d = c.compositions[a] : "function" == typeof c.getComposition ? d = c.getComposition(a) : Muse.Assert.assert(!1, "Could not find any reliable way of obtaining the composition object.");
            Muse.Assert.assert(null != d, "Composition object must not be null.");
            b.stage = d.getStage();
            if (b.stage && "function" == typeof b.stage.setAutoPlay) b.stage.setAutoPlay(!1);
            else
                for (var g in b.stage.timelines) b.stage.autoPlay[g] = !1;
    ...
    l.prototype.update = function (a, b) {
        if (this.enabled)
            if (this.stage) {
                if (this.play !== a.play) !0 === a.play ? (this.play = !0, this.start()) : !1 === a.play ? (this.play = !1, this.stop()) : "number" == typeof a.play ? (this.play = !0, this.seek(b * 1E3 / a.play)) : Muse.Assert.assert(!1, "Unknown widget configuration: play=" + a.play);
                ...
                this.lastInterval = a
            } else this.lastUpdateInterval = a, this.lastUpdateIntervalProgress = b
    };
    ...
    k.MODE_IMMEDIATE = 0;
    k.MODE_DELAYED = 1;
    k.prototype.setModeDelayed = function () {
        if (window.webkitRequestAnimationFrame) this.mode = k.MODE_DELAYED
    };
    k.prototype.getCacheForElement = function (a) {
    ...
    k.prototype.setCSSProperties = function (a, b) {
        var c = this.getCacheForElement(a),
            ...
            f;
        for (f in b)
            if (c.appliedProps[f] !== b[f]) c.queuedProps[f] = b[f], c.hasQueuedProps = d = !0;
        if (!this.requestCSSUpdatePending && d) this.requestCSSUpdatePending = !0, k.MODE_DELAYED == this.mode ? Muse.Utils.requestAnimationFrame(function () {
            g.doCSSUpdate()
        }) : this.doCSSUpdate()
    };
    k.prototype.doCSSUpdate = function () {
    ...
    var i = function () {
        ...
        this.cssProxy = new k(this.getElemID)
    };
    i.TEMP_UID_ATTR = "data-muse-tempuid";
    i.prototype.getEffectProgress = function () {
        return Math.max(0, this.scrollY)
    };
    i.prototype.getHorizontalScroll = function () {
        return this.scrollX - b.offset().left
    };
    i.prototype.getEnvironmentSettings = function () {
        return {
    ...
    i.prototype.onUpdate = function (a, b) {
        var c = 0,
            d, g, f = this.getEnvironmentSettings();
        this.scrollX = a;
        this.scrollY = b;
        for (c = 0; c < this.effects.length; c++) d = this.getEffectProgress(), g = this.getEffectInterval(this.effects[c], d), this.effects[c].update(g, d - g["in"][0], f)
    };
    i.prototype.getEffectInterval = function (a, b) {
        for (var c = 0, d; d = a.data[c]["in"]; c++)
            if (d[0] <
                b && b <= d[1]) return a.data[c];
        Muse.Assert.fail("Why do we have a progress value that does not fit in any interval?");
        return null
    };
    i.prototype.registerEffect = function (a, c, d) {
        var g = this.getElemID(a),
            f = this,
            h = new c(this, a, d);
        if (!1 !== h.enabled) "undefined" == typeof this.initialCSSProperties[g] && (this.initialCSSProperties[g] = {
            ...
            this.initialCSSProperties[g].left, h.elementTop = this.initialCSSProperties[g].top, h.type = c, h.data[0]["in"][0] = -100, h.initialize && h.initialize(), setTimeout(function () {
                var a = f.getEffectProgress(),
                    b = f.getEffectInterval(h, a);
                h.update(b, a - b["in"][0], f.getEnvironmentSettings())
            }, 0), this.effects.push(h)
    };
    i.prototype.getElementEffect = function (a, b) {
        for (var c = n.effects.length, d = 0; d < c; d++) {
            var g = n.effects[d];
            if (g.elem.is(a) && g.type == b) return g
        }
    };
    i.prototype.getElemID = function (a) {
        return a.attr("id") || a.attr(i.TEMP_UID_ATTR) ||
            a.attr(i.TEMP_UID_ATTR, Math.round(Math.random() * 1E6)).attr(i.TEMP_UID_ATTR)
    };
    var n = new i;
    ...
    a.fn.registerOpacityScrollEffect = function (b) {
        return a(this).registerGenericScrollEffect(j,
            b)
    };
    ...
    a.fn.registerEdgeAnimateScrollEffect = function (b) {
        return a(this).registerGenericScrollEffect(l, b)
    };
    ...
    a.fn.clearScrollEffects = function () {
        ...
        for (var b = 0; b < n.effects.length;) n.effects[b].elem.is(this) ? n.effects.splice(b, 1) : b++
    };
    a.fn.cloneScrollEffectsFrom =
        function (a) {
...
(function (a) {
    ...
    j.prototype.onMouseWheel = function () {};
    ...
    h.prototype.available = function () {
        return this.enabled && "ontouchstart" in
            window && jQuery.browser.SafariMobile
    };
    h.prototype.activate = function () {
        a("script").remove();
        g.wrap('<div id="' + h.HTML_WRAPPER_ID + '" />');
        ...
        this.docProps = {
            ...
            paddingLeft: Muse.Utils.getCSSIntValue(g, "margin-left"),
            paddingRight: Muse.Utils.getCSSIntValue(g, "margin-right")
        };
        ...
        this.htmlWrapper.css("min-width", g.outerWidth());
        ...
        a("input,textarea").on("focus",
            this, this.onElementFocus);
    ...
    h.prototype.onElementFocus = function (a) {
        a = a.data;
        if (a.stopTimeout) clearTimeout(a.stopTimeout), a.stopTimeout = 0
    };
    ...
    h.prototype.removeTouchListeners = function () {
        for (var a = 0, b, c = this.touchListeners.length; a < c; a++) b = this.touchListeners[a], b.off("touchstart", this.touchStartHandler), b.off("touchmove", this.touchMoveHandler),
    ...
    h.prototype.onWindowScroll =
        function (a) {
            var a = a.data,
                c = b.scrollLeft(),
                d = b.scrollTop();
            if (!a.inFormFieldEditMode && (0 != c || 0 != d)) window.scrollTo(0, 0), a.scrollTo(c, d)
        };
    ...
    h.prototype.getIFrameScrollOffset = function (b) {
        ...
        if (b.hasClass(h.IFRAME_BODY_CLASS)) return b.data(h.IFRAME_DATA).offset()
    };
    h.prototype.touchStartHandler = function (a) {
        ...
        if (b.canStartScroll(a.target)) b.animation && b.animation.stop(!1, !1), b.speed.y = b.speed.x = 0, a = b.getIFrameScrollOffset(a.target), b.lastTouch.y = c.targetTouches[0].pageY + (a ? a.top : 0), b.lastTouch.x = c.targetTouches[0].pageX + (a ? a.left : 0), b.moveStarted = !0
    };
    h.prototype.touchMoveHandler = function (a) {
        var b = a.data,
            c = a.originalEvent;
        a.preventDefault();
        if (b.moveStarted) a = b.getIFrameScrollOffset(a.target), b.scrollByDelta(b.lastTouch.x - c.targetTouches[0].pageX - (a ? a.left : 0), b.lastTouch.y - c.targetTouches[0].pageY - (a ? a.top : 0)), b.lastTouch.y = c.targetTouches[0].pageY + (a ? a.top : 0), b.lastTouch.x = c.targetTouches[0].pageX + (a ? a.left : 0)
    };
    h.prototype.touchEndHandler = function (b) {
        var c = b.data;
        if (c.moveStarted) {
            c.moveStarted = !1;
            var d = 20 / h.DECELERATION_RATE,
                g = c.speed.x,
                f = c.speed.y,
                b = (1.71 + 0.0020 * Math.sqrt(Math.pow(d * g, 2) + Math.pow(d * f, 2))) / h.DECELERATION_RATE * 1E3 /
                1.71,
                j = 0,
                l = 0;
            c.animation = a({
                ...
                step: function (a) {
                    j = c.decelerate(a);
                    c.scrollByDelta(Math.round((j - l) * d * g), Math.round((j - l) * d * f));
                    l = j
                }
            })
        }
    ...
    var l = function () {
        ...
        this.STANDARD_PROVIDER = new j(this, function (b, c) {
            a.onScroll(b, c)
        });
    ...
    l.prototype.onScroll = function (a, b) {
        for (var c = 0, d = this.updateCallbacks.length; c < d; c++) this.updateCallbacks[c](a,
            b)
    };
    ...
    l.prototype.registerUpdateCallback = function (a) {
    ...
    l.prototype.scrollLeft = function () {
        return this.provider.scrollLeft()
    };
    l.prototype.scrollTo = function (a, b) {
        this.provider.scrollTo(a, b)
    };
    l.prototype.scrollHeight =
        function () {
            return this.provider.scrollHeight()
        };
    l.prototype.scrollWidth = function () {
        ...
        if (-1 != a) {
            ...
            for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
                ...
                if ("generator" == d.getAttribute("name")) {
                    "2015.0.2.310" != d.getAttribute("content") && Muse.assets.outOfDate.push("jquery.scrolleffects.js");
                    break
                }
            }
        }
